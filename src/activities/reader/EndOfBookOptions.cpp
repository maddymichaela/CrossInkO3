...
void EndOfBookOptions::loadOnce(const std::string& currentBookPath) {
  ...
  selector = 0;
  if (!paths.empty()) {
    rowLabels.reserve(paths.size() + 1);
    std::transform(paths.begin(), paths.end(), std::back_inserter(rowLabels), displayName);
...
void EndOfBookOptions::onRowEvent(const fui::ActionEvent& event, void* user) {
  ...
  if (event.value < 0 || event.value > static_cast<int16_t>(self->paths.size())) return;
...
EndOfBookOptions::Action EndOfBookOptions::handleMenuInput(const MappedInputManager& input, std::string* openPath) {
  ...
  if (uiReady.load(std::memory_order_acquire)) {
    ...
    if (snap.touchPressed || snap.touchReleased) {
      ...
      if (event.action == ACTION_ROW && event.value >= 0 && event.value <= static_cast<int16_t>(paths.size())) {
        const auto tappedRow = static_cast<size_t>(event.value);
        if (tappedRow < paths.size()) {
          if (openPath) {
            *openPath = paths[tappedRow];
          }
